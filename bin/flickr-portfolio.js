(async function () {
    ...
    const {
        FLICKR_API_KEY,
        FLICKR_USER_ID,
        FLICKR_COLLECTION_ID,
        BIG_DATA_CLOUD_API_KEY,
    } = process.env;
    const flickr = new Flickr(FLICKR_API_KEY);
    const geoClient = require('@bigdatacloudapi/client')(
        BIG_DATA_CLOUD_API_KEY
    );
    ...
    await flickr.collections
        ...
        .then(function (res) {
            console.log(
                `Found ${res.body.collections.collection[0].set.length} albums in collection`
            );
    ...
    for (const album of portfolio.albums) {
        ...
        await flickr.photosets
            ...
            .then(function (res) {
                console.log(
                    `Found ${res.body.photoset.photo.length} photos in album ${album.title}`
                );
                    ...
                    if (photo.latitude && photo.longitude) {
                        geoClient
                            ...
                            .then((jsonResult) => {
                                ...
                                if (photo.country) {
                                    const existingPlaces = portfolio.places.map(
                                        (place) => place.name
                                    );

                                    // create place if it doesn't already exist
                                    if (
                                        !existingPlaces.includes(photo.country)
                                    ) {
                                        portfolio.places.push({
                                            name: photo.country,
                                            photos: [],
                                            count: 0,
                                        });
                            ...
                            .catch(function (error) {
                                console.error(
                                    'Error getting country from coordinates: ',
                                    error
                                );
                            });
                    }
