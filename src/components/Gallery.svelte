    ...
    for (const keyEntry of keyMap) {
      if (event.key === keyEntry.key) {
        const targetElement = document.querySelector(
          `${location.hash} > a.${keyEntry.selector}`
        );
        targetElement.click();
      }
    }
...
<div id="gallery" class="gallery">
  {#each photoIds as photoId}
    <a class="gallery-item" href="#lightbox-{photoId}">
      <img
        class="gallery-image"
        src={getPhotoSource(photoId, "url_z")}
        alt=""
        loading="lazy"
      />
    </a>
    <div class="lightbox" id="lightbox-{photoId}">
      ...
      <a class="slideshow-nav next" href="#lightbox-{getNextPhotoId(photoId)}"
        >&#8594;</a
      >
      <a class="slideshow-nav close" href={"#"}>X</a>
      <a
        class="slideshow-nav prev"
        href="#lightbox-{getPreviousPhotoId(photoId)}">&#8592;</a
      >
    </div>
  {/each}
</div>
